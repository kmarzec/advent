...
struct Program
{
    ...
    std::weak_ptr<Program> parent;
    std::vector<std::shared_ptr<Program>> children;
};
typedef std::shared_ptr<Program> ProgramPtr;


ProgramPtr BuildTree(const std::string& input)
{
    ...
    for (std::string line; std::getline(inputStream, line);)
    {
        char name[256];
        int weight;
        int itemsParsed = sscanf(line.c_str(), "%s (%d)", name, &weight);
        assert(itemsParsed == 2);

        ProgramPtr program = std::make_shared<Program>();
        program->name = name;
        program->weigth = weight;

        std::string::size_type childListPos = line.find("->");
        if (childListPos != std::string::npos)
        {
            std::string childListStr = line.substr(childListPos +2);
            std::string::size_type b = 0;
            while ((b = childListStr.find_first_not_of(", ", b)) != std::string::npos)
            {
                std::string::size_type e = childListStr.find_first_of(", ", b);
                program->childrenNames.emplace_back(childListStr.substr(b, e - b));
                b = e;
            }
        }
    ...
    {
        for (const std::string& child : kv.second->childrenNames)
        {
            auto it = programs.find(child);
            assert(it != programs.end());
            //if (it != programs.end())
            {
                it->second->parent = kv.second;
                kv.second->children.push_back(it->second);
            }
        }
    }
...
int ValidateWeight(ProgramPtr program)
{
    ...
    std::map<int, std::pair<int, ProgramPtr>> weightOccurences;
...
void Day7_2(const std::string& task, const std::string& input)
{
...
void Day7_1(const std::string& task, const std::string& input)
{
    ProgramPtr root = BuildTree(input);
    assert(root);
...
{
    std::string input1 =
        R"(mypyemv (1058) -> tdssotr, pebnvks, zaulju
snslv (48)
aytbgf (30)
mkcrsy (42)
shwcd (42)
...
std::string input2 =
        R"(jztwlhz (111) -> xfzbans, brlrqy, zdguboh
